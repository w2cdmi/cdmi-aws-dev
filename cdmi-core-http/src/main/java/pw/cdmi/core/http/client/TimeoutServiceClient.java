...
public class TimeoutServiceClient extends DefaultServiceClient {
    ...
    public ResponseMessage sendRequestCore(ServiceClient.Request request, ExecutionContext context)
        ...
        try {
        	...
        	logger.error("[ExecutorService]The current thread was interrupted while waiting: ", e);
            ...
            logger.error("[ExecutorService]The computation threw an exception: ", ex);
            throw ex;
        } catch (TimeoutException e) {
        	logger.error("[ExecutorService]The wait " + this.config.getRequestTimeout() + " timed out: ", e);
            
            httpRequest.abort();
            throw new ClientException(e.getMessage(), ClientErrorCode.CONNECTION_TIMEOUT, "Unknown", e);
        }
    ...
    public void shutdown() {
        ...
        try {
            if (!executor.awaitTermination(ClientConfiguration.DEFAULT_THREAD_POOL_WAIT_TIME, TimeUnit.MILLISECONDS)) {
                ...
                if (!executor.awaitTermination(ClientConfiguration.DEFAULT_THREAD_POOL_WAIT_TIME, TimeUnit.MILLISECONDS)) {
                	logger.warn("Pool did not terminate in " + 

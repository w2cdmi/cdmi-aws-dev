...
public class RepeatableInputStream extends InputStream {
	private static final Logger logger = LoggerFactory.getLogger(RepeatableInputStream.class);
    ...
    public void reset() throws IOException {
        if (bytesReadFromMark <= bufferSize) {
        	logger.debug("Reset after reading " + bytesReadFromMark + " bytes.");
            bufferOffset = 0;
        } else {
    ...
    public int read(byte[] out, int outOffset, int outLength) throws IOException {
        ...
        if (bytesReadFromMark + count <= bufferSize) {
        	...
        	logger.debug("Buffer size " + bufferSize + " has been exceeded and the input stream "
